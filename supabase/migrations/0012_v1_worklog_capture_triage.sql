...
BEGIN
  ...
  IF p_action = 'worklog' THEN
    v_worklog_text := COALESCE(
      NULLIF(BTRIM(p_note_body), ''),
      NULLIF(BTRIM(v_inbox.raw_text), ''),
      NULLIF(BTRIM(v_inbox.transcript_text), '')
    );

    IF v_worklog_text IS NULL THEN
      RAISE EXCEPTION 'empty_worklog_body';
    END IF;
    ...
    RETURNING id INTO v_worklog_id;
